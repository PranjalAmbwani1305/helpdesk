...
def clear_old_vectors():
    """Deletes all existing vectors from Pinecone before inserting new ones."""
    index.delete(delete_all=True)
    st.info("Cleared old vectors from Pinecone.")
...
def store_vectors(chapters, articles, pdf_name):
    ...
    for i, article in enumerate(articles):
        st.write(f"Storing Article {article['title']}: {article['content'][:200]}...")  # Debugging
...
if index_name not in pc.list_indexes():
    st.warning("Pinecone index does not exist. Creating it now...")
    pc.create_index(index_name, dimension=384)  # 384 is the embedding size for MiniLM
index = pc.Index(index_name)
