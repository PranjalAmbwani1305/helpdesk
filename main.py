...
if index_name not in [i['name'] for i in pc.list_indexes()]:
    pc.create_index(
        name=index_name,
        dimension=1536,  # ✅ Keeping 1536
        metric="cosine"
    )

# ✅ Use 'BAAI/bge-large-en' for 1536 dimensions
...
def store_vectors(chunks, pdf_name):
    for i, chunk in enumerate(chunks):
        try:
            index.add_texts([str(chunk)], metadatas=[{"pdf_name": pdf_name}])
        except Exception as e:
            st.error(f"Error adding vectors to Pinecone: {e}")

def query_vectors(query, selected_pdf):
    try:
        results = index.similarity_search(query, k=5, filter={"pdf_name": {"$eq": selected_pdf}})
