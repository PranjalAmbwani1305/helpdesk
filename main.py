...
stored_pdfs = [match["metadata"]["pdf_name"] for match in index.query(vector=[0]*1536, top_k=10, include_metadata=True).get("matches", [])] if index else []
selected_pdf = None
...
elif pdf_source == "Choose from the Document Storage" and stored_pdfs:
    selected_pdf = st.sidebar.selectbox("Select a stored document:", stored_pdfs)
